...
class LLC2d(gym.Env):
    ...
    def reset(self):
        ...
        while inarea:
            ...
            inarea = self.inblock(self.goalx, self.goalx)
    ...
    def render(self, mode='human'):
        if self.viewer == None:
            ...
            for ii in range(blocknum):
                blockplace = rendering.FilledPolygon(
                    [(self.blockax[ii] + 500, self.blockby[ii] + 500), (self.blockax[ii] + 500, self.blockay[ii] + 500),
                     (self.blockbx[ii] + 500, self.blockay[ii] + 500),
                     (self.blockbx[ii] + 500, self.blockby[ii] + 500)])
                self.viewer.add_geom(blockplace)

            # self.label1 = pyglet.text.Label(str(self.goal[0])+"，"+str(self.goal[1]), font_size=36,
            #                       x=self.goal[0]+500, y=self.goal[1]+500, anchor_x='left', anchor_y='bottom',
            #                       color=(255, 123, 255, 255))
            # self.label2 = pyglet.text.Label(str(self.placex)+"，"+str(self.placey), font_size=36,
            #                       x=self.placex+500, y=self.placey+500, anchor_x='left', anchor_y='bottom',
            #                       color=(255, 123, 255, 255))
            # self.label1.text = "%i  %i" % (self.goal[0],self.goal[1])
            # self.label1.x = self.goal[0]
            # self.label1.y = self.goal[1]
            # self.label2.text = "%i  %i" % (self.placex, self.placey)
            # self.label2.x = self.placex
            # self.label2.y = self.placey
            # self.label1.draw()
            # self.label2.draw()
        self.cartrans2.set_translation(self.goal[0] + 500, self.goal[1] + 500)
        self.cartrans1.set_translation(self.placex + 500, self.placey + 500)
        text1 = self.viewer.draw_text("目标位置[%i %i]" % (self.goal[0], self.goal[1]),
                                      position=(self.goal[0] + 500, self.goal[1] + 500))
        text2 = self.viewer.draw_text("现在位置[%i %i]" % (self.placex, self.placey),
                                      position=(self.placex + 500, self.placey + 500))
        self.viewer.add_geom(text1)
        self.viewer.add_geom(text2)
