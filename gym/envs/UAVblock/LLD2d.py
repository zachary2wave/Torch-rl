...
class LLD2d(gym.Env):
    def __init__(self):
        ...
        "无人机位置 x,y ，无人机速度 vx,vy， 目标 gx，gy "
    ...
    def step(self, a):
        ...
        for i in range(len(self.v)):
            tempbound = self.bound(self.a[i] * self.delta + self.v[i], self.Vmax, -self.Vmax)
        ...
        self.are[0].append(self.a[0])
        self.are[1].append(self.a[1])
        self.retime.append(self.time)
        self.rere.append(reward)
        return S_, reward, done, {'G0':np.sum(self.G0)}
# 判断生成的点是否在可行范围内
...
# 计算dB
    ...
    def show_animation(self, save_gif=None):
        ...
        plt.scatter(self.goalx, self.goaly, marker="*")
        plt.xlim(-400, 400)
        plt.ylim(-400, 400)

        trajectory, = plt.plot([], [], c='r')
        arrow = ax.annotate("", xy=(0, 0),
                            xytext=(0, 0), arrowprops=dict(arrowstyle="->"))
        time_text = ax.text(-390, 390, '')
        tra = self.tra
        are = self.are
        rere = self.rere
        ...
        def update(i):
            if i < len(self.tra[0]):
                ...
                arrow.xy = (tra[0][i] + are[0][i] * 10, tra[1][i] + are[1][i] * 10)
                arrow.xyann = (tra[0][i], tra[1][i])
                time_text.set_text("time = "+str(i)+ "\treward = "+str(rere[i]))
                return trajectory, arrow, time_text
            else:
                plt.close(fig)
        ani = animation.FuncAnimation(fig, update, frames=range(len(self.tra[0])),
    ...
    def show_(self):
        ...
        for loop in range(len(self.tra[0])):
            line =ax.plot(self.tra[0][0:loop], self.tra[1][0:loop], c='r'),
            arrow = ax.arrow(self.tra[0][loop], self.tra[1][loop], self.are[0][i], self.are[1][i])
            plt.pause(0.01)
            del line
            del arrow
    ...
    def show_gym_render(self):
        ...
        if self.viewer is None:
            ...
            block = []
            for i in range(len(self.blockstartx)):
                block.append(rendering.FilledPolygon([(self.blockstartx[i],  self.blockstarty[i]),
                                           (self.blockstartx[i], self.blockstarty[i]+self.blockl[i]),
                                           (self.blockstartx[i]+self.blockw[i], self.blockstarty[i]+self.blockl[i]),
                                           (self.blockstartx[i]+self.blockw[i], self.blockstarty[i])])
                            )
                self.viewer.add_geom(block[-1])
            aim_mark = rendering.make_circle(3, filled=False)
            aim_mark.set_color(.5, .5, .5)
            aim_mark.add_attr(rendering.Transform(translation=(self.placex, self.placey)))
            self.aim_transform = rendering.Transform()
            aim_mark.add_attr(self.aim_transform)
            self.viewer.add_geom(aim_mark)

            uav_mark = rendering.make_circle(3, filled=True)
            uav_mark.set_color(0, 0, 0)
            uav_mark.add_attr(rendering.Transform(translation=(self.goalx, self.goaly)))
