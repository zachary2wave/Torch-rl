...
class Downlink_D2Ddy(gym.Env):
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-150, high=150, shape=(4*self.NSP+5,), dtype=np.float32)
    ...
    def reset(self):
        ...
        temp = np.concatenate(([x /400 for x in repositionx], [y/400 for y in  repositiony],
                               [g/300 for g in self.G],[snr/10 for snr in self.SNR]), axis=0)
    ...
    def step(self, a):
        ...
        #         self.G[i] += np.random.uniform(0, 1, 1)
        self.step_time += 1
        ...
        # if self.step_time == 3000:
        if teskleft == 0 or self.step_time == 10000:
            self.done = 1
            # reward += self.totalR/10
        ...
        if self.placey < -400:
            # reward += -10e-2 * np.maximum(abs(self.placey) - 400, 0)-0.1
            reward += -10
            if self.boundary == 1:
                self.placey = -400
            if self.show == 1:
