...
class CL2dwithblock(gym.Env):
    ...
    def reset(self):
        ...
        inarea = 1
        while inarea:
            self.placex= np.random.randint(-self.MAXboundary , self.MAXboundary , 1)  # 节点位置x
            self.placey = np.random.randint(-self.MAXboundary , self.MAXboundary , 1)  # 节点位置y
            inarea = self.inblock(self.placex, self.placey)
        self.time = 0
        ...
        S = np.array([self.placex/self.MAXboundary, self.placey/self.MAXboundary])
    ...
    def step(self, a):
        ...
        if tp4:
            ...
            olddis = sum(abs([self.goalx-self.placex, self.goaly-self.placey]))
            self.placex = (1 - tp1) * x + tp1 * self.placex,  # 无人机位置x
            self.placey = (1 - tp2) * y + tp2 * self.placey  # 无人机位置y
            # 判断所链接的用户
            dis = sum(abs([self.goalx-self.placex, self.goaly-self.placey]))
            # 判断信噪比
            reward += olddis-dis
        ...
        S_ = np.array([self.placex/self.MAXboundary, self.placey/self.MAXboundary])
    ...
    def inblock(self,x,y):
        ...
        for time in range(blocknum):
            if self.blockstartx[time] < x < self.blockstartx[time]+self.blockw[time] \
                    and self.blockbstarty[time] < y < self.blockbstarty[time]+self.blockl[time]:
                flag = 1
                break
        return flag
