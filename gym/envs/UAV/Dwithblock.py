...
class Dwithblock(gym.Env):
    ...
    def step(self, a):
        ...
        outbound1 = self.inblock(self.placex + self.v[0]*self.delta,self.placey + self.v[1]*self.delta)
        outbound2 = self.placebound(self.placex + self.v[0] * self.delta, self.placey + self.v[1] * self.delta)
        print(outbound1,outbound2)
        ...
        return S_, reward/100, self.done, {'total': self.totalR/self.totalP, 'Gleft': teskleft/np.sum(self.Grecord + 1),
                                       'output':output,'powerused':P + PS,'G0':np.sum(self.G0)}
# 判断生成的点是否在可行范围内
        ...
        for time in range(0, self.NSP):
            ...
            for bl in range(len(self.blockax)):
                ...
                if dot1>0 and dot2>0 and dot3>0 and dot4>0:
                    ...
                    flag = 1
            PLmax.append(10 * math.log10(D**self.alpha) + self.lossb + flag*inta_los + (1-flag) * inta_Nlos)
        return PLmax, D
