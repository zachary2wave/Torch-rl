...
class D2dwithblock(gym.Env):
    ...
    def step(self, a):
        ...
        self.P = self.Pmax
        P = 10**(self.P/10)/1000
        outbound = self.speedbound(self.v[0]+self.a[0]*self.delta, self.v[1]+self.a[1]*self.delta)
        if outbound:
            reward += -(outbound) * 10
        else:
            self.v = [self.v[i] + self.a[i] * self.delta for i in range(2)]
        x = self.placex + self.v[0] * self.delta + 0.5 * self.a[0] * (self.delta**2)
        y = self.placey + self.v[1] * self.delta + 0.5 * self.a[1] * (self.delta**2)
        outbound1 = self.inblock(x, y)
        outbound2 = self.placebound(x, y)
        if outbound1+outbound2:
            reward += -(outbound1+outbound2) * 10
        else:
            self.placex = x  # 无人机位置x
            self.placey = y  # 无人机位置y
            # 判断所链接的用户
            self.rate, self.SNR, self.ref = self.Rate()
            self.cline = np.argmax(self.rate)
            # 判断信噪比
    ...
    def Rate(self):
        ...
        for i in range(self.NSP):
            ...
            for bl in range(len(self.blockstartx)):
                dot1 = np.sign(K * (self.blockstartx[bl] - self.placex)
                               - (self.blockstarty[bl] - self.placey))

                dot2 = np.sign(K * (self.blockstartx[bl]+self.blockw[bl] - self.placex)
                               - (self.blockstarty[bl] - self.placey))

                dot3 = np.sign(K * (self.blockstartx[bl] - self.placex)
                               - (self.blockstarty[bl]+self.blockl[bl] - self.placey))

                dot4 = np.sign(K * (self.blockstartx[bl]+self.blockw[bl] - self.placex)
                               - (self.blockstarty[bl]+self.blockl[bl] - self.placey))
