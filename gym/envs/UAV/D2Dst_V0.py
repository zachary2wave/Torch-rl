...
class Downlink_D2Dst(gym.Env):
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-50, high=150, shape=(2*self.NSP+5,), dtype=np.float32)
    ...
    def reset(self):
        ...
        # self.SPplacex = np.random.randint(-200, 200, self.NSP)  # 节点位置x
        # self.SPplacey = np.random.randint(-200, 200, self.NSP)  # 节点位置y
        # self.G = np.random.uniform(10, 30, self.NSP)
    ...
    def step(self, a):
        ...
        if teskleft == 0 or self.step_time == 500:
            self.done = 1
            total = sum(self.G0) / self.totalP
            # reward += sum(self.G0)
        # 限制最大用时
        # if self.time == self.T:
        #     self.done = 1
        ...
        self.time += 1
        return S_, reward, self.done, {'total': total*10000, 'temp': self.totalR/(1+self.totalP)}
