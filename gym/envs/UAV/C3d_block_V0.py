...
class C3d_block_V0(gym.Env):
    def __init__(self):
        ...
        self.amaxz = 5
        self.MAXboundary, self.MAXboundarylowz, self.MAXboundaryhighz = 400, 40, 100
        ...
        # 定义动作
        self.action_space = spaces.Box(low=np.array([-self.amax, -self.amax, -self.amaxz]),\
                                       high=np.array([self.amax, self.amax, self.amaxz]),\
                                       dtype=np.float32)
        #定义状态空间
        "利用了用户的信噪比 和 数据量 + 无人机位置 x,y,z 和 无人机速度 vx,vy,vz "
        self.observation_space = spaces.Box(low=-150, high=150, shape=(2 * self.NSP + 6,), dtype=np.float32)
        # 定义输出的变量
        self.show = 0
        self.boundary = 1
        self.eps = 0.001
    ...
    def reset(self):
            ...
            S = np.append(S, [self.place[0] / self.MAXboundary,
                              self.place[1] / self.MAXboundary,
                              self.place[2] / (self.MAXboundaryhighz-self.MAXboundarylowz)])
    ...
    def step(self, a):
        ...
        tp3 = self.bound(z, self.MAXboundaryhighz, self.MAXboundarylowz)
        ...
        S_ = np.append(S_, [self.place[0] / self.MAXboundary,
                            self.place[1] / self.MAXboundary,
                            self.place[2] / (self.MAXboundaryhighz-self.MAXboundarylowz)])
