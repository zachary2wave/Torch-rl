import gym
import time
from Torch_rl.algorithm.PPO_Lagrangian import PPO_LAGRANGIAN_Agent
...
#%%
policy_model =  LSTM_Dense_Hin(env.observation_space.shape[0], env.action_space.shape[0],
                        lstm_unit=64, lstm_layer=1, dense_layer=[64],
                 hidden_activate=nn.Tanh())
value_model = DenseNet(env.observation_space.shape[0], 1,
                  hidden_activate=nn.Tanh(), hidden_layer=[64, 64])

Agent = PPO_LAGRANGIAN_Agent(env, policy_model, value_model, gamma=0.90,
